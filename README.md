# ComfyUI 远程面板（Android）V2.1

一个面向“手机 + Termius/Tailscale/SSH 转发 + 远程 ComfyUI”的中文 Android 控制端。

## V2.1 重点升级

V2.1 在 V2.0 多输出工作流基础上，重点改善手机端高频操作：**输入图片按工作流长期保留、一次多选并按顺序批量入队、首页默认精简、高级设置折叠、工作流列表，以及自定义背景/启动页与 3 种 Launcher 图标切换**。详见 `V2.1_CHANGELOG.md`。
...
- 工作流正文保存在 App 私有目录 `workflows_v18`；V2.1 还在每个工作流配置里保存输入图片 URI 与替换开关。
